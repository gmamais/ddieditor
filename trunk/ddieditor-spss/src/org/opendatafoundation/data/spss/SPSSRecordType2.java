...
public class SPSSRecordType2 extends SPSSAbstractRecordType {
	...
	public void read(SPSSFile is) throws IOException, SPSSFileException {
		...
		if (recordTypeCode != 2)
			throw new SPSSFileException(
					"Error reading variable Record: bad record type ["
							+ recordTypeCode + "]. Expecting Record Type 2.");
		// variableRecord type
		...
		if (Math.abs(missingValueFormatCode) > 3)
			throw new SPSSFileException(
					"Error reading variable Record: invalid missing value format code ["
							+ missingValueFormatCode + "]. Range is -3 to 3.");
	...
	public String toString() {
		...
		str += "\n- Type          : " + printFormatType + " ["
				+ getFormatTypeCode(printFormatType) + "/"
		...
		str += "\n- Type          : " + writeFormatType + " ["
				+ getFormatTypeCode(writeFormatType) + "/"
		...
		if (this.missingValueFormatCode > 0) {
			...
			for (int i = 0; i < this.missingValueFormatCode; i++) {
				str += "\nMissing " + i + "       :";
				if (this.variableTypeCode == 0)
					str += "" + SPSSUtils.byte8ToDouble(missingValue[i]);
				else
					str += SPSSUtils.byte8ToString(missingValue[i]);
			}
			...
			if (this.variableTypeCode == 0)
				...
				str += SPSSUtils.byte8ToString(missingValue[0]);
			str += " - ";
			if (this.variableTypeCode == 0)
				str += "" + SPSSUtils.byte8ToDouble(missingValue[1]);
			else
				str += SPSSUtils.byte8ToString(missingValue[1]);
			if (this.missingValueFormatCode == -3) {
				str += "\nMissing 3       : ";
				if (this.variableTypeCode == 0)
					str += "" + SPSSUtils.byte8ToDouble(missingValue[2]);
				else
					str += SPSSUtils.byte8ToString(missingValue[2]);
			}
		}
		return (str);
	}
